...
namespace imageptrwinform
{
    public partial class Form1 : Form
    {
        ...
        [DllImport("gdi32.dll")]
        private static extern int StretchDIBits(IntPtr hdc, int XDest, int YDest, int DestWidth, int DestHeight, int XSrc, int YSrc, int SrcWidth, int SrcHeight, IntPtr lpBits, ref BITMAPINFO lpbmi, int iUsage, int rop);
        [DllImport("gdi32.dll")]
        private static extern int SetStretchBltMode(IntPtr hdc, int mode);

        private Altaircam cam_ = null;
        private Altaircam.ImagePtr ptr_ = new Altaircam.ImagePtr();
        private uint count_ = 0;
        private int[] pixelFormatArray_ = null;
        private BITMAPINFO bmi_ = new BITMAPINFO();
        ...
        {
            Altaircam.ImagePtr ptr = new Altaircam.ImagePtr();
            if (cam_.PullImagePtr(0, ref ptr)) {
                if (ptr_.id != 0)
                    cam_.PushImagePtr(ptr_.id);
                ptr_ = ptr;
        ...
        {
            Altaircam.ImagePtr ptr = new Altaircam.ImagePtr();
            if (cam_.PullImagePtr(1, ref ptr))
            {
                Altaircam.FrameInfoV4 info = Altaircam.GetFrameInfoV4(ptr);
                Bitmap bmp = new Bitmap((int)info.v3.width, (int)info.v3.height, Altaircam.TDIBWIDTHBYTES((int)info.v3.width * 24), PixelFormat.Format24bppRgb, ptr.ptr8);
                bmp.Save(string.Format("imageptrwinform_{0}.jpg", ++count_), ImageFormat.Jpeg);
                cam_.PushImagePtr(ptr.id);
            }
        }
        ...
        public Form1()
        {
            ...
            bmi_.bmiHeader.biSize = Marshal.SizeOf(typeof(BITMAPINFOHEADER));
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            radioButton1.Enabled = false;
            radioButton2.Enabled = false;
            trackBar2.SetRange(Altaircam.TEMP_MIN, Altaircam.TEMP_MAX);
            trackBar3.SetRange(Altaircam.TINT_MIN, Altaircam.TINT_MAX);
            Altaircam.GigeEnable(null);
            Altaircam.CtiEnable(null, null);
        }

        private void DelegateOnEventCallback(Altaircam.eEVENT evt)
        {
            /* this is call by internal thread of altaircam.dll which is NOT the same of UI thread.
        ...
        private void startDevice(string camId)
        {
            ...
            if (cam_ != null)
            {
                ...
                int noutputmode = 0;
                if (cam_.IoControl(1, Altaircam.eIoControType.IOCONTROLTYPE_GET_OUTPUTMODE, 0, out noutputmode))
                {
                ...
                //TEC
                int tecMode = 0;
                if (cam_.get_Option(Altaircam.eOPTION.OPTION_TEC, out tecMode))
                {
                    ...
                    if (tecMode == 1)
                    {
                        ...
                        int nTec;
                        cam_.get_Option(Altaircam.eOPTION.OPTION_TECTARGET, out nTec);
                ...
                if (cam_.get_eSize(out eSize))
                {
                    ...
                    pixelFormatArray_ = new int[pixelFormatnum];
                    for (sbyte i = 0; i < pixelFormatnum; ++i)
                    {
                        int pixelFormatval;
                        cam_.get_PixelFormatSupport(i, out pixelFormatval);
                        pixelFormatArray_[i] = pixelFormatval;
                        comboBox2.Items.Add(Altaircam.PixelFormatName((Altaircam.ePIXELFORMAT)pixelFormatval));
                    }
                    comboBox2.SelectedIndex = 0;

                    if (cam_.get_Size(out bmi_.bmiHeader.biWidth, out bmi_.bmiHeader.biHeight))
                    {
                         ...
                         */
                        if (!cam_.StartPullModeWithCallback(new Altaircam.DelegateEventCallback(DelegateOnEventCallback)))
        ...
        {
            int tecRange = 0;
            if (cam_.get_Option(Altaircam.eOPTION.OPTION_TECTARGET_RANGE, out tecRange))
        ...
        private void OnSnap(object sender, EventArgs e)
        {
            if (cam_ != null)
            {
                if (cam_.StillResolutionNumber <= 0) {
                    if (ptr_.ptr8 != IntPtr.Zero) {
                        Bitmap bmp = new Bitmap(bmi_.bmiHeader.biWidth, bmi_.bmiHeader.biHeight, ((bmi_.bmiHeader.biWidth * 3 + 3) / 4) * 4, PixelFormat.Format24bppRgb, ptr_.ptr8);
                        bmp.Save(string.Format("imageptrwinform_{0}.jpg", ++count_), ImageFormat.Jpeg);
                    }
                }
                else {
        ...
        private void OnSelectResolution(object sender, EventArgs e)
        {
            ...
            {
                uint eSize = 0;
                if (cam_.get_eSize(out eSize) && (eSize != comboBox1.SelectedIndex))
                {
                    cam_.Stop();
                    ...
                    if (cam_.get_Size(out bmi_.bmiHeader.biWidth, out bmi_.bmiHeader.biHeight))
                        cam_.StartPullModeWithCallback(new Altaircam.DelegateEventCallback(DelegateOnEventCallback));
                }
            }
        }
        ...
        private void OnSelectPixelFormat(object sender, EventArgs e)
        {
            if (cam_ != null)
            {
                ...
                cam_.put_Option(Altaircam.eOPTION.OPTION_PIXEL_FORMAT, pixelFormatArray_[pixelFormat]);
            }
        }
        ...
        {
            int tecMode = 0;
            cam_.get_Option(Altaircam.eOPTION.OPTION_TEC, out tecMode);
            if (tecMode == 1)
            {
                button4.Text = "TEC ON";
                trackBar4.Enabled = false;
                cam_.put_Option(Altaircam.eOPTION.OPTION_TEC, 0);
            }
            else
            {
                button4.Text = "TEC OFF";
                trackBar4.Enabled = true;
                cam_.put_Option(Altaircam.eOPTION.OPTION_TEC, 1);
            }
        }
        ...
        {
            int nval;
            if (cam_.IoControl(0, Altaircam.eIoControType.IOCONTROLTYPE_GET_TRIGGERSOURCE, 0, out nval))
            {
                cam_.put_Option(Altaircam.eOPTION.OPTION_TRIGGER, 2);
            ...
            {
                cam_.put_Option(Altaircam.eOPTION.OPTION_TRIGGER, 1);
                nval = 5;
                comboBox3.Enabled = false;
                button5.Enabled = true;
            }
        ...
        private void OnTriggerSourceSelect(object sender, EventArgs e)
        {
            ...
            cam_.IoControl(0, Altaircam.eIoControType.IOCONTROLTYPE_SET_TRIGGERSOURCE, nval);
            if (nval == 5)
                button5.Enabled = true;
            else
                button5.Enabled = false;
        ...
        private void OnOutputModeSelect(object sender, EventArgs e)
        {
            ...
            cam_.IoControl(1, Altaircam.eIoControType.IOCONTROLTYPE_SET_OUTPUTMODE, nval);
        }
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            if (ptr_.ptr8 != IntPtr.Zero) {
                ...
                StretchDIBits(hdc, 0, 0, pictureBox1.ClientRectangle.Width, pictureBox1.ClientRectangle.Height,
                        0, 0, bmi_.bmiHeader.biWidth, bmi_.bmiHeader.biHeight,
                        ptr_.ptr8,
                        ref bmi_, 0, 0x00CC0020);
                g.ReleaseHdc();
                g.DrawString(label4.Text, this.Font, SystemBrushes.ControlText, 0, 0);
            }
        }
    }
}
