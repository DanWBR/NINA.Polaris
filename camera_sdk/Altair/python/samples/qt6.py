...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.pData = None
        self.res = 0
        self.temp = altaircam.ALTAIRCAM_TEMP_DEF
        self.tint = altaircam.ALTAIRCAM_TINT_DEF
        self.count = 0
        ...
        self.lbl_temp = QLabel(str(altaircam.ALTAIRCAM_TEMP_DEF))
        self.lbl_tint = QLabel(str(altaircam.ALTAIRCAM_TINT_DEF))
        self.slider_temp = QSlider(Qt.Orientation.Horizontal)
        self.slider_tint = QSlider(Qt.Orientation.Horizontal)
        self.slider_temp.setRange(altaircam.ALTAIRCAM_TEMP_MIN, altaircam.ALTAIRCAM_TEMP_MAX)
        self.slider_temp.setValue(altaircam.ALTAIRCAM_TEMP_DEF)
        self.slider_tint.setRange(altaircam.ALTAIRCAM_TINT_MIN, altaircam.ALTAIRCAM_TINT_MAX)
        self.slider_tint.setValue(altaircam.ALTAIRCAM_TINT_DEF)
    ...
    def startCamera(self):
        self.pData = bytes(altaircam.TDIBWIDTHBYTES(self.imgWidth * 24) * self.imgHeight)
        ...
        if self.cur.model.flag & altaircam.ALTAIRCAM_FLAG_MONO == 0:
            self.handleTempTintEvent()
        try:
            self.hcam.StartPullModeWithCallback(self.eventCallBack, self)
        except altaircam.HRESULTException:
            self.closeCamera()
            QMessageBox.warning(self, "Warning", "Failed to start camera.")
        else:
            self.cmb_res.setEnabled(True)
            self.cbox_auto.setEnabled(True)
            self.btn_autoWB.setEnabled(self.cur.model.flag & altaircam.ALTAIRCAM_FLAG_MONO == 0)
            self.slider_temp.setEnabled(self.cur.model.flag & altaircam.ALTAIRCAM_FLAG_MONO == 0)
            self.slider_tint.setEnabled(self.cur.model.flag & altaircam.ALTAIRCAM_FLAG_MONO == 0)
    ...
    def openCamera(self):
        self.hcam = altaircam.Altaircam.Open(self.cur.id)
        if self.hcam:
            ...
            with QSignalBlocker(self.cmb_res):
                ...
                self.cmb_res.setEnabled(True)
            self.hcam.put_Option(altaircam.ALTAIRCAM_OPTION_BYTEORDER, 0) #Qimage use RGB byte order
    ...
    def onBtnOpen(self):
        if self.hcam:
            ...
            arr = altaircam.Altaircam.EnumV2()
            if 0 == len(arr):
        ...
        if self.hcam:
            if 0 == self.cur.model.still:    # not support still image capture
                if self.pData is not None:
                    image = QImage(self.pData, self.imgWidth, self.imgHeight, altaircam.TDIBWIDTHBYTES(self.imgWidth * 24), QImage.Format.Format_RGB888)
                    self.count += 1
    ...
    def eventCallBack(nEvent, self):
        '''callbacks come from altaircam.dll/so internal threads, so we use qt signal to post this event to the UI thread'''
        self.evtCallback.emit(nEvent)

    def onevtCallback(self, nEvent):
        '''this run in the UI thread'''
        if self.hcam:
            if altaircam.ALTAIRCAM_EVENT_IMAGE == nEvent:
                self.handleImageEvent()
            elif altaircam.ALTAIRCAM_EVENT_EXPOSURE == nEvent:
                self.handleExpoEvent()
            elif altaircam.ALTAIRCAM_EVENT_TEMPTINT == nEvent:
                self.handleTempTintEvent()
            elif altaircam.ALTAIRCAM_EVENT_STILLIMAGE == nEvent:
                self.handleStillImageEvent()
            elif altaircam.ALTAIRCAM_EVENT_ERROR == nEvent:
                self.closeCamera()
    ...
    def handleStillImageEvent(self):
        info = altaircam.AltaircamFrameInfoV3()
        try:
            ...
            if info.width > 0 and info.height > 0:
                buf = bytes(altaircam.TDIBWIDTHBYTES(info.width * 24) * info.height)
                try:
                    self.hcam.PullImageV3(buf, 1, 24, 0, info)
                except altaircam.HRESULTException:
                    pass
                else:
                    image = QImage(buf, info.width, info.height, altaircam.TDIBWIDTHBYTES(info.width * 24), QImage.Format.Format_RGB888)
                    self.count += 1
                    image.save("pyqt{}.jpg".format(self.count))

if __name__ == '__main__':
    altaircam.Altaircam.GigeEnable(None, None)
