...
static void on_Expotime(int pos, void*) {
    if (pos * 1000 < g_minExpoTime)
        Altaircam_put_ExpoTime(g_hcam, g_minExpoTime);
    else
        Altaircam_put_ExpoTime(g_hcam, pos * 1000);
}
...
int main(int, char**)
{
    ...
    else
    {
        ...
        else
        {
            ...
            Altaircam_get_ExpTimeRange(g_hcam, &g_minExpoTime, &g_maxExpoTime, &g_defExpoTime);
