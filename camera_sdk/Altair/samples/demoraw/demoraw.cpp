...
static void __stdcall EventCallback(unsigned nEvent, void* pCallbackCtx)
{
    if (ALTAIRCAM_EVENT_IMAGE == nEvent)
    {
        ...
        const HRESULT hr = Altaircam_PullImageV4(g_hcam, g_pImageData, 0, 0, 0, &info);
        if (FAILED(hr))
    ...
    {
        AltaircamFrameInfoV4 info = { 0 };
        const HRESULT hr = Altaircam_PullImageV4(g_hcam, g_pImageData, 1, 0, 0, &info);
        if (FAILED(hr))
...
int main(int, char**)
{
    ...
    HRESULT hr = Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_RAW, -1); /* use raw image data */
    printf("put option raw: hr = 0x%08x\n", hr);

    Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_BITDEPTH, 0); /* enable bitdepth */

    int nMaxWidth = 0, nMaxHeight = 0;
    hr = Altaircam_get_Resolution(g_hcam, 0, &nMaxWidth, &nMaxHeight);
    if (FAILED(hr))
