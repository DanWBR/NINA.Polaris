...
static void __stdcall EventCallback(unsigned nEvent, void* pCallbackCtx)
{
    ...
    {
        AltaircamFrameInfoV4 info = { 0 };
        HRESULT hr = Altaircam_PullImageV4(g_hcam, nullptr, 1, 24, 0, &info); //peek width & height
        if (FAILED(hr))
        ...
        else
        {
            ...
            {
                /* After we get the image data, we can do anything for the data we want to do */
                hr = Altaircam_PullImageV4(g_hcam, pStillImage, 1, 24, 0, &info);
                if (FAILED(hr))
                    printf("failed to pull still image, hr = 0x%08x\n", hr);
                else
            ...
            else
            {
                ...
                do {
                    ...
                    if (fgets(str, 1023, stdin))
                    {
                        if (('x' == str[0]) || ('X' == str[0]))
                            break;
                        Altaircam_SnapN(g_hcam, atoi(str), 1);
                    }
                } while (true);
            }
        }
