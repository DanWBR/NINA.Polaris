...
int main(int, char**)
{
    ...
    {
        if (arr[i].model->flag & ALTAIRCAM_FLAG_TRIGGER_EXTERNAL)
        {
#if defined(_WIN32)
            printf("%ls\n", arr[i].displayname);
#else
            printf("%s\n", arr[i].displayname);
#endif
            g_hcam = Altaircam_Open(arr[i].id);
            if (NULL == g_hcam)
            {
    ...
    else
    {
        ...
        {
            Altaircam_put_ExpoTime(g_hcam, 250000);
            hr = Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_TRIGGER, 2);
            if (FAILED(hr))
            ...
            else
            {
                ...
                do {
                    if (fgets(str, 1023, stdin))
                    {
                        ...
                        {
                            Altaircam_IoControl(g_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_TRIGGERSOURCE, trigger_source, NULL);//select trigger source
                            Altaircam_IoControl(g_hcam, 2, ALTAIRCAM_IOCONTROLTYPE_SET_GPIODIR, 0x01, NULL);//GPIO_0 -> output
                            Altaircam_IoControl(g_hcam, 3, ALTAIRCAM_IOCONTROLTYPE_SET_GPIODIR, 0x01, NULL);//GPIO_1 -> output
                            Altaircam_IoControl(g_hcam, 1, ALTAIRCAM_IOCONTROLTYPE_SET_OUTPUTMODE, 0x00, NULL);//opt_out -> Trigger wait signal
                            Altaircam_IoControl(g_hcam, 2, ALTAIRCAM_IOCONTROLTYPE_SET_OUTPUTMODE, 0x01, NULL);//GPIO_0 -> Exposure effective signal
                            Altaircam_IoControl(g_hcam, 3, ALTAIRCAM_IOCONTROLTYPE_SET_OUTPUTMODE, 0x02, NULL);//GPIO_1 -> Strobe signal
                            Altaircam_IoControl(g_hcam, 3, ALTAIRCAM_IOCONTROLTYPE_SET_STROBEDELAYMODE, 0x01, NULL);//strobe delay mode
                            Altaircam_IoControl(g_hcam, 3, ALTAIRCAM_IOCONTROLTYPE_SET_STROBEDELAYTIME, 500000, NULL);//strobe delay time
                            break;
                        }
                    }
                } while (true);
