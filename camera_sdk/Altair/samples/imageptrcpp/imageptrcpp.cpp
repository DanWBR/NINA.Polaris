...
static BOOL SaveImageRaw(const wchar_t* strFilename, const void* pData, unsigned width, unsigned height, unsigned char bitDepth)
{
	FILE* fp = _wfopen(strFilename, L"wb");
	if (fp)
	{
		fwrite(pData, 1, width * height * (bitDepth > 8 ? 2 : 1), fp);
		fclose(fp);
		return TRUE;
	}
	return FALSE;
...
class CSnapDlg : public CDialogImpl<CSnapDlg>
{
	friend class CMainFrame;
	unsigned m_nNum;
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			{
				unsigned char* tmpBuffer = (unsigned char*)alloca(uLength);
				HRESULT hr = Altaircam_read_UART(m_hcam, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Altaircam_write_UART(m_hcam, tmpBuffer, uLength);
				wchar_t strMessage[256];
...
class CMainFrame : public CFrameWindowImpl<CMainFrame>, public CUpdateUI<CMainFrame>
{
	...
	BOOL			m_bPaused;
	int				m_nSnapType; // 0-> not snaping, 1 -> single snap, 2 -> multiple snap, 3 -> multiple snap raw
	unsigned		m_nSnapSeq;
	...
	BEGIN_MSG_MAP_EX(CMainFrame)
		...
		COMMAND_RANGE_HANDLER_EX(ID_SNAPR_RESOLUTION0, ID_SNAPR_RESOLUTION4, OnSnapRResolution)
	...
	BEGIN_UPDATE_UI_MAP(CMainFrame)
		...
		UPDATE_ELEMENT(ID_SNAPR_RESOLUTION0, UPDUI_MENUPOPUP)
		UPDATE_ELEMENT(ID_SNAPR_RESOLUTION1, UPDUI_MENUPOPUP)
		UPDATE_ELEMENT(ID_SNAPR_RESOLUTION2, UPDUI_MENUPOPUP)
		UPDATE_ELEMENT(ID_SNAPR_RESOLUTION3, UPDUI_MENUPOPUP)
		UPDATE_ELEMENT(ID_SNAPR_RESOLUTION4, UPDUI_MENUPOPUP)
...
public:
	CMainFrame()
	: m_hcam(NULL), m_bPaused(FALSE), m_nSnapType(0), m_nSnapSeq(0), m_nSnapFile(0), m_pWmvRecord(NULL), m_view(this)
	{
...
private:
	...
	void OnPreviewResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (SUCCEEDED(Altaircam_get_eSize(m_hcam, &eSize)))
		{
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				...
				{
					if (m_ptr.id)
						Altaircam_PushImagePtr(m_hcam, m_ptr.id);
					memset(&m_ptr, 0, sizeof(m_ptr));
	...
	void OnSnapRResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		if (NULL == m_hcam)
			return;
		...
		if ((IDOK == dlg.DoModal()) && (dlg.m_nNum > 0))
		{
			if (SUCCEEDED(Altaircam_SnapR(m_hcam, nID - ID_SNAPR_RESOLUTION0, dlg.m_nNum)))
			{
				m_nSnapType = 3;
				m_nSnapSeq = dlg.m_nNum;
				UpdateSnapMenu();
	...
	void OnEventImage()
	{
		...
		HRESULT hr = Altaircam_PullImagePtr(m_hcam, 0, &ptr);
		if (FAILED(hr))
			return;
		if ((ptr.ptrInfo->v3.width != m_header.biWidth) || (ptr.ptrInfo->v3.height != m_header.biHeight))
		{
			Altaircam_PushImagePtr(m_hcam, ptr.id);
			return;
		}
		if (m_ptr.id)
			Altaircam_PushImagePtr(m_hcam, m_ptr.id);
		...
		UpdateFrameInfoText(m_ptr.ptrInfo);
		if (m_pWmvRecord)
			m_pWmvRecord->WriteSample(m_ptr.ptr8);
	}

	void OnEventSnap()
	{
		AltaircamImagePtr ptr = { 0 };
		HRESULT hr = Altaircam_PullImagePtr(m_hcam, 1, &ptr);
		if (SUCCEEDED(hr))
		{
			if (3 == m_nSnapType)
			{
				wchar_t strPath[MAX_PATH];
				swprintf(strPath, L"%04u.raw", m_nSnapFile++);
				SaveImageRaw(strPath, ptr.ptrRaw, ptr.ptrInfo->v3.width, ptr.ptrInfo->v3.height, Altaircam_get_PixelFormatBitdepth(ptr.pixelFormat));
			}
			else
			{
				...
				header.biWidth = ptr.ptrInfo->v3.width;
				header.biHeight = ptr.ptrInfo->v3.height;
				header.biSizeImage = TDIBWIDTHBYTES(header.biWidth * header.biBitCount) * header.biHeight;
				if (2 == m_nSnapType)
				{
					wchar_t strPath[MAX_PATH];
					swprintf(strPath, L"%04u.jpg", m_nSnapFile++);
					SaveImageByWIC(strPath, ptr.ptr8, &header);
				}
				else
				{
					if (PathMatchSpec(m_szFilePath, L"*.bmp"))
						SaveImageBmp(m_szFilePath, ptr.ptr8, &header);
					else
						SaveImageByWIC(m_szFilePath, ptr.ptr8, &header);
				}
			}
	...
	void CloseDevice()
	{
		...
		memset(&m_ptr, 0, sizeof(m_ptr)); /* Frame buffers that have not been returned become invalid immediately after Altaircam_Close */
	...
	void OnDeviceChanged()
	{
		...
		CMenuHandle snaprsubmenu = submenu.GetSubMenu(3);
		...
		while (snaprsubmenu.GetMenuItemCount() > 0)
			snaprsubmenu.RemoveMenu(snaprsubmenu.GetMenuItemCount() - 1, MF_BYPOSITION);
		...
		if (NULL == m_hcam)
		{
			...
			snaprsubmenu.AppendMenu(MF_STRING | MF_GRAYED, ID_SNAP_RESOLUTION0, L"Empty");
		...
		else
		{
			...
			for (unsigned i = 0; i < m_dev.model->preview; ++i)
			{
				...
				snaprsubmenu.AppendMenu(MF_STRING, ID_SNAPR_RESOLUTION0 + i, res);
	...
	void UpdateSnapMenu()
	{
		if (m_nSnapType)
		{
			for (unsigned i = 0; i < m_dev.model->preview; ++i)
			{
				...
				UIEnable(ID_SNAPR_RESOLUTION0 + i, FALSE);
			}
			return;
		}
		...
		if (SUCCEEDED(Altaircam_get_eSize(m_hcam, &eSize)))
		{
			for (unsigned i = 0; i < m_dev.model->preview; ++i)
			{
				if (m_dev.model->still == m_dev.model->preview) /* still capture full supported */
				{
					...
					UIEnable(ID_SNAPR_RESOLUTION0 + i, TRUE);
				}
				else if (0 == m_dev.model->still) /* still capture not supported */
				{
					...
					UIEnable(ID_SNAPR_RESOLUTION0 + i, (eSize == i) ? TRUE : FALSE);
				}
