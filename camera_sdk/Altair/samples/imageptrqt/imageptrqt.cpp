...
MainWidget::MainWidget(QWidget* parent)
    ...
    , m_res(0), m_temp(ALTAIRCAM_TEMP_DEF), m_tint(ALTAIRCAM_TINT_DEF), m_count(0)
{
    ...
    QGroupBox* gboxexp = new QGroupBox("Exposure");
    {
        ...
        connect(m_cbox_auto, &QCheckBox::stateChanged, this, [this](bool state)
        {
            if (m_hcam)
            {
                Altaircam_put_AutoExpoEnable(m_hcam, state ? 1 : 0);
        ...
        connect(m_slider_expoTime, &QSlider::valueChanged, this, [this](int value)
        {
            if (m_hcam)
            {
                ...
                if (!m_cbox_auto->isChecked())
                   Altaircam_put_ExpoTime(m_hcam, value);
            }
        });
        connect(m_slider_expoGain, &QSlider::valueChanged, this, [this](int value)
        {
            if (m_hcam)
            {
                ...
                if (!m_cbox_auto->isChecked())
                    Altaircam_put_ExpoAGain(m_hcam, value);
            }
        });
    ...
    QGroupBox* gboxwb = new QGroupBox("White balance");
    {
        ...
        {
            Altaircam_AwbOnce(m_hcam, nullptr, nullptr);
        });
        m_lbl_temp = new QLabel(QString::number(ALTAIRCAM_TEMP_DEF));
        m_lbl_tint = new QLabel(QString::number(ALTAIRCAM_TINT_DEF));
        m_slider_temp = new QSlider(Qt::Horizontal);
        m_slider_tint = new QSlider(Qt::Horizontal);
        m_slider_temp->setRange(ALTAIRCAM_TEMP_MIN, ALTAIRCAM_TEMP_MAX);
        m_slider_temp->setValue(ALTAIRCAM_TEMP_DEF);
        m_slider_tint->setRange(ALTAIRCAM_TINT_MIN, ALTAIRCAM_TINT_MAX);
        m_slider_tint->setValue(ALTAIRCAM_TINT_DEF);
        ...
        connect(m_slider_temp, &QSlider::valueChanged, this, [this](int value)
        {
            m_temp = value;
            if (m_hcam)
                Altaircam_put_TempTint(m_hcam, m_temp, m_tint);
    ...
    connect(this, &MainWidget::evtCallback, this, [this](unsigned nEvent)
    {
        ...
        if (m_hcam)
        {
            if (ALTAIRCAM_EVENT_IMAGE == nEvent)
                ...
                handleExpoEvent();
            else if (ALTAIRCAM_EVENT_TEMPTINT == nEvent)
                handleTempTintEvent();
...
void MainWidget::startCamera()
{
    ...
    Altaircam_get_ExpoAGainRange(m_hcam, &usmin, &usmax, &usdef);
    m_slider_expoGain->setRange(usmin, usmax);
    if (0 == (m_cur.model->flag & ALTAIRCAM_FLAG_MONO))
        handleTempTintEvent();
    handleExpoEvent();
    if (SUCCEEDED(Altaircam_StartPullModeWithCallback(m_hcam, eventCallBack, this)))
    {
        ...
        m_btn_autoWB->setEnabled(0 == (m_cur.model->flag & ALTAIRCAM_FLAG_MONO));
        m_slider_temp->setEnabled(0 == (m_cur.model->flag & ALTAIRCAM_FLAG_MONO));
        m_slider_tint->setEnabled(0 == (m_cur.model->flag & ALTAIRCAM_FLAG_MONO));
        m_btn_open->setText("Close");
        m_btn_snap->setEnabled(true);

        int bAuto = 0;
        Altaircam_get_AutoExpoEnable(m_hcam, &bAuto);
...
void MainWidget::onBtnSnap()
{
    if (m_hcam)
    {
        if (0 == m_cur.model->still)    // not support still image capture
        {
            if (m_ptr.ptr8)
            {
                QImage image(m_ptr.ptr8, m_imgWidth, m_imgHeight, QImage::Format_RGB888);
                image.save(QString::asprintf("imageptrqt_%u.jpg", ++m_count));
            }
        }
        else
        {
...
{
    AltaircamImagePtr ptr;
    if (SUCCEEDED(Altaircam_PullImagePtr(m_hcam, 0, &ptr)))
    {
        if (m_ptr.id)
            Altaircam_PushImagePtr(m_hcam, m_ptr.id);
        m_ptr = ptr;
        QImage image(m_ptr.ptr8, m_ptr.ptrInfo->v3.width, m_ptr.ptrInfo->v3.height, QImage::Format_RGB888);
...
void MainWidget::handleExpoEvent()
{
    ...
    Altaircam_get_ExpoAGain(m_hcam, &gain);
    {
...
{
    AltaircamImagePtr ptr;
    if (SUCCEEDED(Altaircam_PullImagePtr(m_hcam, 1, &ptr))) // peek
    {
        QImage image(ptr.ptr8, ptr.ptrInfo->v3.width, ptr.ptrInfo->v3.height, QImage::Format_RGB888);
        image.save(QString::asprintf("imageptrqt_%u.jpg", ++m_count));
    }
}
