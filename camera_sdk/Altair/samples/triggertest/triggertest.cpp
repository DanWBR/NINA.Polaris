...
{
	HAltaircam m_hcam;
	const AltaircamModelV2* m_model;
...
private:
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		else
		{
			...
			Altaircam_get_ExpTimeRange(m_hcam, &minTime, &maxTime, &defTime);
			...
			Altaircam_put_ExpoTime(m_hcam, expoTime);
			if (FAILED(Altaircam_StartPullModeWithCallback(m_hcam, StaticCameraCallback, this)))
			{
...
private:
	...
	void CameraCallback(unsigned nEvent)
	{
		if (ALTAIRCAM_EVENT_IMAGE == nEvent)
		{
			...
			const HRESULT hr = Altaircam_PullImageV4(m_hcam, pdata, 0, 24, 0, nullptr);
			if (SUCCEEDED(hr))
			{
				++m_image;
				if (m_save)
	...
	void OpenCamera(const wchar_t* camId, const AltaircamModelV2* model)
	{
		...
		m_model = model;
		if (0 == (m_model->flag & ALTAIRCAM_FLAG_TRIGGER_SOFTWARE))
		{
