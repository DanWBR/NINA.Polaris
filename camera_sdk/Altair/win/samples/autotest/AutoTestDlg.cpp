...
BOOL CAutoTestDlg::OnInitDialog()
{
	...
	Altaircam_GigeEnable(HotPlug, GetSafeHwnd());
	Altaircam_CtiEnable(HotPlug, GetSafeHwnd(), NULL);
...
LRESULT CAutoTestDlg::OnMsgCamevent(WPARAM wp, LPARAM lp)
{
	...
	case ALTAIRCAM_EVENT_STILLIMAGE:
		OnEventStillImage();
		break;
	case ALTAIRCAM_EVENT_HEARTBEAT:
...
void CAutoTestDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (2 == nIDEvent)
	{
		...
		if (g_hcam)
		{
			...
			if (SUCCEEDED(Altaircam_get_FrameRate(g_hcam, &nFrame, &nTime, &nTotalFrame)) && nTime)
	...
	else if (3 == nIDEvent)
	{
		...
		{
			short nTemperature;
			if (SUCCEEDED(Altaircam_get_Temperature(g_hcam, &nTemperature)))
...
LRESULT CAutoTestDlg::OnPreviewResChanged(WPARAM wp, LPARAM lp)
{
	...
	HRESULT hr = Altaircam_get_eSize(g_hcam, &nResolutionIndex);
	if (FAILED(hr))
		return -1;

	if (nResolutionIndex != nSel)
	{
		hr = Altaircam_Stop(g_hcam);
		if (FAILED(hr))
			return -1;
		Altaircam_put_eSize(g_hcam, nSel);
...
{
	g_cameraCnt = Altaircam_EnumV2(g_cam);
...
void CAutoTestDlg::OnBnClickedButtonStart()
{
	...
	if (0 == startBtnText.Compare(_T("Open")))
	{
		...
		g_hcam = Altaircam_Open(g_cur.id);
		if (nullptr == g_hcam)
			return;

		m_dwHeartbeat = 0;
		if (g_HeartbeatTimeout && (g_cur.model->flag & ALTAIRCAM_FLAG_EVENT_HARDWARE))
		{
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_EVENT_HARDWARE, 1);
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_EVENT_HARDWARE | ALTAIRCAM_EVENT_HEARTBEAT, 1);

			SetTimer(1, 1000, nullptr);
		}
		Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_NOPACKET_TIMEOUT, g_NopacketTimeout);
		Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_NOFRAME_TIMEOUT, g_NoframeTimeout);
		Altaircam_put_RealTime(g_hcam, g_bRealtime ? 1 : 0);
...
void CAutoTestDlg::StartCamera()
{
	...
	Altaircam_get_Size(g_hcam, &width, &height);
	...
	Altaircam_StartPullModeWithWndMsg(g_hcam, m_hWnd, MSG_CAMEVENT);
}
...
void CAutoTestDlg::OnEventImage()
{
	...
	HRESULT hr = Altaircam_PullImageV4(g_hcam, m_pImageData, 0, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		if (g_bROITest && g_bROITest_SnapStart)
		{
			unsigned offsetX = 0, offsetY = 0, width = 0, height = 0;
			Altaircam_get_Roi(g_hcam, &offsetX, &offsetY, &width, &height);
		...
		else if (g_bImageSnap)
		{
			...
			Altaircam_get_Size(g_hcam, &resWidth, &resHeight);
			CString str;
			SYSTEMTIME tm;
			GetLocalTime(&tm);
		...
		else if (g_bBitdepthTest)
		{
			...
			Altaircam_get_Size(g_hcam, &resWidth, &resHeight);
			Altaircam_get_Option(g_hcam, ALTAIRCAM_OPTION_PIXEL_FORMAT, &iFormat);
			const char* formatName = Altaircam_get_PixelFormatName(iFormat);
			CString str;
			SYSTEMTIME tm;
			GetLocalTime(&tm);
...
{
	AltaircamFrameInfoV4 info = { 0 };
	HRESULT hr = Altaircam_PullImageV4(g_hcam, nullptr, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.v3.width * 24) * info.v3.height);
		hr = Altaircam_PullImageV4(g_hcam, pData, 1, 24, 0, nullptr);
		if (SUCCEEDED(hr))
		{
			BITMAPINFOHEADER header = { 0 };
			header.biSize = sizeof(header);
...
void CAutoTestDlg::OpenCloseCamera(unsigned nRes)
{
	...
	if (0 == startBtnText.Compare(_T("Open")))
	{
		...
		Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_NOFRAME_TIMEOUT, g_NoframeTimeout);
		Altaircam_put_RealTime(g_hcam, g_bRealtime ? 1 : 0);
		Altaircam_put_eSize(g_hcam, nRes);
		StartCamera();
...
void CAutoTestDlg::OnBnClickedButton1()
{
	...
	Altaircam_Replug(g_cam[m_camList.GetCurSel()].id);
}
...
void COptionsDlg::OnOK()
{
	...
	if (g_NopacketTimeout && (g_NopacketTimeout < ALTAIRCAM_NOPACKET_TIMEOUT_MIN))
	{
	...
	if (g_NoframeTimeout && (g_NoframeTimeout < ALTAIRCAM_NOFRAME_TIMEOUT_MIN))
	{
