...
BOOL CBitDepthPropertyPage::OnInitDialog()
{
	...
	if (g_hcam)
	{
		...
		Altaircam_get_PixelFormatSupport(g_hcam, -1, &cnt);
		for (int i = 0; i < cnt; ++i)
		{
			CString str;
			Altaircam_get_PixelFormatSupport(g_hcam, i, &iFormat);
			const char* name = Altaircam_get_PixelFormatName(iFormat);
			str.Format(L"%S", name);
		...
		for (int i = 0; i < cnt; ++i)
		{
			...
			Altaircam_get_PixelFormatSupport(g_hcam, i, &val);
			if (val == iFormat)
				nCur = i;
		}
...
void CBitDepthPropertyPage::OnCbnSelchangeComboBitdepth()
{
	...
	Altaircam_get_PixelFormatSupport(g_hcam, idx, &iFormat);
