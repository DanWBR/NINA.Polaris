...
CBitDepthTestPropertyPage::CBitDepthTestPropertyPage()
	...
	, m_formatCount(0), m_interval(1000)
{
}
...
BEGIN_MESSAGE_MAP(CBitDepthTestPropertyPage, CPropertyPage)
	...
	ON_WM_TIMER()
	ON_EN_CHANGE(IDC_EDIT_BITDEPTH_INTERVAL, &CBitDepthTestPropertyPage::OnEnChangeEditBitdepthInterval)
END_MESSAGE_MAP()
...
BOOL CBitDepthTestPropertyPage::OnInitDialog()
{
	...
	UpdateHint();
	GetDlgItem(IDC_BUTTON_BITDEPTH_TEST_START)->EnableWindow(m_totalCount > 0 && m_interval >= 100);
	SetDlgItemInt(IDC_EDIT_BITDEPTH_TEST_CNT, m_totalCount, FALSE);
	SetDlgItemInt(IDC_EDIT_BITDEPTH_INTERVAL, m_interval, FALSE);
	return TRUE;
}
...
void CBitDepthTestPropertyPage::OnEnChangeEditBitdepthInterval()
{
	m_interval = GetDlgItemInt(IDC_EDIT_BITDEPTH_INTERVAL);
	GetDlgItem(IDC_BUTTON_BITDEPTH_TEST_START)->EnableWindow(m_totalCount > 0 && m_interval >= 100);
}
...
void CBitDepthTestPropertyPage::OnTimer(UINT_PTR nIDEvent)
{
	...
	Altaircam_get_PixelFormatSupport(g_hcam, -1, &pixelNum);
	Altaircam_get_PixelFormatSupport(g_hcam, m_formatCount, &iFormat);
	Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_PIXEL_FORMAT, iFormat);
