...
BOOL CCGPropertyPage::OnInitDialog()
{
	...
	if (g_cur.model->flag & ALTAIRCAM_FLAG_CG)
	{
		int val = -1;
		Altaircam_get_Option(g_hcam, ALTAIRCAM_OPTION_CG, &val);
		if (val == 0)
		{
			m_ConversionGain = 2;
		...
		if (g_cur.model->flag & ALTAIRCAM_FLAG_CGHDR)
			GetDlgItem(IDC_RADIO_MCG)->SetWindowText(L"HDR");
		else if (g_cur.model->flag & ALTAIRCAM_FLAG_GHOPTO)
...
void CCGPropertyPage::OnBnClickedRadioCG()
{
	...
	if (g_hcam)
	{
		if (m_ConversionGain == 0)
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_CG, 1);
		else if (m_ConversionGain == 1)
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_CG, 2);
		else if (m_ConversionGain == 2)
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_CG, 0);
	}
}
