...
BOOL CCoolingPropertyPage::OnInitDialog()
{
	...
	if (g_cur.model->flag & ALTAIRCAM_FLAG_TEC_ONOFF)
	{
		int Target = -1;
		BOOL bEnableTEC = FALSE;
		Altaircam_get_Option(g_hcam, ALTAIRCAM_OPTION_TEC, &bEnableTEC);
		...
		if (bEnableTEC)
		{
			Altaircam_get_Option(g_hcam, ALTAIRCAM_OPTION_TECTARGET, &Target);
			CString str;
...
void CCoolingPropertyPage::OnBnClickedButtonApply()
{
	if (g_hcam)
	{
		...
		if (nullptr == endptr || '\0' == *endptr)
			Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_TECTARGET, (int)(d * 10));
		else
...
{
	if (g_hcam)
		Altaircam_put_Option(g_hcam, ALTAIRCAM_OPTION_TEC, IsDlgButtonChecked(IDC_CHECK_TEC) ? 1 : 0);
