...
void CExposureGainPropertyPage::OnAutoExposure()
{
	if (GetSafeHwnd())
	{
		...
		{
			unsigned time = 0;
			Altaircam_get_ExpoTime(g_hcam, &time);
		...
		if (GetDlgItem(IDC_SLIDER_GAIN))
		{
			...
			Altaircam_get_ExpoAGain(g_hcam, &gain);
			SetGainValue(gain);
		}
	}
}
...
{
	int bAutoExp = 0;
	Altaircam_get_AutoExpoEnable(g_hcam, &bAutoExp);
...
{
	Altaircam_put_AutoExpoEnable(g_hcam, ((CButton*)GetDlgItem(IDC_CHECK_AUTO))->GetCheck() ? 1 : 0);
	UpdateSlidersEnable();
}
...
void CExposureGainPropertyPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER_TARGET))
	{
		...
		Altaircam_get_AutoExpoTarget(g_hcam, &curTarget);
		unsigned short target = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->GetPos();
		if (target != curTarget)
		{
			Altaircam_put_AutoExpoTarget(g_hcam, target);
	...
	{
		unsigned curTime = 0;
		Altaircam_get_ExpoTime(g_hcam, &curTime);
		unsigned time = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->GetPos();
		if (time != curTime)
		{
			Altaircam_put_ExpoTime(g_hcam, time);
			CString str;
	...
	else if (pScrollBar == GetDlgItem(IDC_SLIDER_GAIN))
	{
		...
		Altaircam_get_ExpoAGain(g_hcam, &curGain);
		unsigned short gain = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->GetPos();
		if (gain != curGain)
		{
			Altaircam_put_ExpoAGain(g_hcam, gain);
...
void CExposureGainPropertyPage::OnEnKillfocusEditExpotime()
{
	...
	if (!strTemp.IsEmpty())
	{
		...
		if (nullptr == endptr || '\0' == *endptr)
		{
			...
			Altaircam_get_ExpTimeRange(g_hcam, &nMinExpoTime, &nMaxExpoTime, &nDefExpoTime);
			if (value < nMinExpoTime)
				value = nMinExpoTime;
			else if (value > nMaxExpoTime)
				value = nMaxExpoTime;
			Altaircam_put_ExpoTime(g_hcam, value);
...
void CExposureGainPropertyPage::OnEnKillfocusEditExpogain()
{
	...
	if (!strTemp.IsEmpty())
	{
		...
		Altaircam_get_ExpoAGainRange(g_hcam, &nMinExpoAGain, &nMaxExpoAGain, &nDefExpoAGain);
		int value = GetDlgItemInt(IDC_EDIT_GAIN);
		if (value < nMinExpoAGain)
			value = nMinExpoAGain;
		else if (value > nMaxExpoAGain)
			value = nMaxExpoAGain;
		Altaircam_put_ExpoAGain(g_hcam, value);
