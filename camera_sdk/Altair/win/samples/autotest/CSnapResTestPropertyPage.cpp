...
void CSnapResTestPropertyPage::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		Stop();
		AfxMessageBox(_T("Snap and resolution test completed."), MB_OK | MB_ICONINFORMATION);
		return;
	}
	if (m_snap == 0)
	...
	g_bSnapFinish = false;
	g_snapCount = m_count;
	Altaircam_Snap(g_hcam, m_snap);
	if (++m_snap >= Altaircam_get_StillResolutionNumber(g_hcam))
	{
		m_snap = 0;
		if (++m_resCount >= Altaircam_get_ResolutionNumber(g_hcam))
		{
			m_resCount = 0;
			++m_count;
			UpdateHint();
