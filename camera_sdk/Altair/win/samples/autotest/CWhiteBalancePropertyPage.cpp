...
void CWhiteBalancePropertyPage::OnWhiteBalance()
{
	...
	{
		int temp = 0, tint = 0;
		Altaircam_get_TempTint(g_hcam, &temp, &tint);
		SetTempValue(temp);
		SetTintValue(tint);
	}
}
...
BOOL CWhiteBalancePropertyPage::OnInitDialog()
{
	...
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TEMP))->SetRange(ALTAIRCAM_TEMP_MIN, ALTAIRCAM_TEMP_MAX);
	SetTempValue(ALTAIRCAM_TEMP_DEF);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TINT))->SetRange(ALTAIRCAM_TINT_MIN, ALTAIRCAM_TINT_MAX);
	SetTintValue(ALTAIRCAM_TINT_DEF);
...
void CWhiteBalancePropertyPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	{
		Altaircam_get_TempTint(g_hcam, &curTemp, &curTint);
		int temp = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TEMP))->GetPos();
		if (temp != curTemp)
		{
			Altaircam_put_TempTint(g_hcam, temp, curTint);
	...
	else if (pScrollBar == GetDlgItem(IDC_SLIDER_TINT))
	{
		...
		if (tint != curTint)
		{
			Altaircam_put_TempTint(g_hcam, curTemp, tint);
...
{
	Altaircam_AwbOnce(g_hcam, nullptr, nullptr);
}
