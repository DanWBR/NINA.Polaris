...
void CdemoafDlg::OnBnClickedButton1()
{
	...
	Altaircam_get_Revision(m_hcam, &m_revision);
...
void CdemoafDlg::StartAFLensControll()
{
	...
	Altaircam_put_AFMode(m_hcam, AltaircamAFMode_CALIBRATE, 0, 0, 0);
}
...
void CdemoafDlg::OnCbnSelchangeCombo1()
{
	...
	if (nResolutionIndex != nSel)
	{
		...
		StartDevice();
		Altaircam_put_AFMode(m_hcam, AltaircamAFMode_MANUAL, 0, 0, 0);
	}
}
...
void CdemoafDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		AltaircamAFState afStatus;
		Altaircam_get_AFState(m_hcam, &afStatus);
		if (afStatus.AF_Mode == AltaircamAFMode_AUTO || afStatus.AF_Mode == AltaircamAFMode_ONCE)
			Altaircam_put_AFRoi(m_hcam, 0, 0, 0, 0);
...
void CdemoafDlg::UpdateExpoSlidersEnable()
{
	...
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->SetRange(ALTAIRCAM_AETARGET_MIN, ALTAIRCAM_AETARGET_MAX);
	unsigned short target = 0;
	Altaircam_get_AutoExpoTarget(m_hcam, &target);
	...
	Altaircam_get_ExpoAGainRange(m_hcam, &gainMin, &gainMax, &gainDef);
...
{
	unsigned m_nTime = 0;
	Altaircam_get_ExpoTime(m_hcam, &m_nTime);
...
{
	USHORT nGain = 0;
	Altaircam_get_ExpoAGain(m_hcam, &nGain);
...
CRect CdemoafDlg::SetDisplayLimit(CRect rect)
{
	CRect display_limit;
	int iWidth, iHeight;
	HREFTYPE hr = Altaircam_get_Size(m_hcam, &iWidth, &iHeight);
...
void CdemoafDlg::SetFrameRateLimit()
{
	...
	Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_FRAMERATE, nFrameRateLimit);
}
...
void CdemoafDlg::OnEventImage()
{
	...
	HRESULT hr = Altaircam_PullImageV4(m_hcam, m_pImageData, 0, 24, 0, &info);
	Altaircam_get_FrameRate(m_hcam, &m_nFrame, &m_nTime, &m_nTotalFrame);
	if (SUCCEEDED(hr))
	{
		const CRect rc = GetDrawRect();
		CClientDC dc(this);
...
{
	AltaircamFrameInfoV4 info = { 0 };
	HRESULT hr = Altaircam_PullImageV4(m_hcam, NULL, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.v3.width * 24) * info.v3.height);
		hr = Altaircam_PullImageV4(m_hcam, pData, 1, 24, 0, NULL);
		if (SUCCEEDED(hr))
		{
...
void CdemoafDlg::SetClarityRect()
{
	...
	Altaircam_put_AFRoi(m_hcam, m_ClarityROI.usOffset_X, m_ClarityROI.usOffset_Y, m_ClarityROI.usSize_X, m_ClarityROI.usSize_Y);
}
...
{
	UpdateData(TRUE);
	Altaircam_get_LensInfo(m_hcam, &m_afLensInfo);
	...
	case IDC_SLIDER_TARGET:
	{
		...
		Altaircam_get_AutoExpoTarget(m_hcam, &curTarget);
		if (target != curTarget)
		{
			Altaircam_put_AutoExpoTarget(m_hcam, target);
			SetDlgItemInt(IDC_STATIC_TARGET, target);
		}
	}
	case IDC_SLIDER_EXP://Auto Exposure
	{
		...
		Altaircam_get_ExpoTime(m_hcam, &curTime);
		if (time != curTime)
		{
			Altaircam_put_ExpoTime(m_hcam, time * 1000);
	...
	case IDC_SLIDER_GAIN://Auto Exposure
	{
		...
		Altaircam_get_ExpoAGain(m_hcam, &curGain);
		unsigned short gain = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->GetPos();
		if (gain != curGain)
		{
			Altaircam_put_ExpoAGain(m_hcam, gain);
	...
	case IDC_SLIDER_AP://aperture
	{
		...
		Altaircam_put_AFAperture(m_hcam, SliderApSet);
	...
	case IDC_SLIDER_FOCUS://Manual slider for autofocus
	{
		...
		Altaircam_put_AFFMPos(m_hcam, SliderFocusSet);
		break;
	}
	}
...
void CdemoafDlg::OnBnClickedFocusmotorup()
{
	...
	Altaircam_put_AFFMPos(m_hcam, pos + step);
...
void CdemoafDlg::OnTimer(UINT_PTR nIDEvent)
{
	if ((1 == nIDEvent) && m_hcam)
	{
		...
		UpdateData(TRUE);
		AltaircamAFState afStatus;
		if (SUCCEEDED(Altaircam_get_LensInfo(m_hcam, &m_afLensInfo)) && SUCCEEDED(Altaircam_get_AFState(m_hcam, &afStatus)))
		{
			...
			{
				if (afStatus.AF_Mode == AltaircamAFMode_MANUAL)
				{
...
void CdemoafDlg::OnCbnSelchangeComboF()
{
	...
	Altaircam_put_AFAperture(m_hcam, cindex);
...
{
	Altaircam_put_AFMode(m_hcam, AltaircamAFMode_AUTO, 0, 0, 0);
	SetFocusFNControl(FALSE);
	GetDlgItem(IDC_LENSCAL)->EnableWindow(TRUE);
	GetDlgItem(IDC_RADIO_MANUAL)->EnableWindow(TRUE);
	GetDlgItem(IDC_BUTTON_ONEPUSH)->EnableWindow(FALSE);
...
void CdemoafDlg::OnBnClickedButtonOnepush()
{
	AltaircamAFState afStatus;
	Altaircam_get_AFState(m_hcam, &afStatus);
	if (afStatus.AF_Mode == AltaircamAFMode_ONCE)
	{
		Altaircam_put_AFMode(m_hcam, AltaircamAFMode_MANUAL, 0, 0, 0);
		GetDlgItem(IDC_BUTTON_ONEPUSH)->SetWindowText(L"Once");
	}
	...
	{
		Altaircam_put_AFMode(m_hcam, AltaircamAFMode_ONCE, 0, 0, 0);
		((CButton*)GetDlgItem(IDC_RADIO_MANUAL))->SetCheck(FALSE);
		((CButton*)GetDlgItem(IDC_RADIO_AUTO))->SetCheck(FALSE);
		SetFocusFNControl(FALSE);
		GetDlgItem(IDC_LENSCAL)->EnableWindow(FALSE);
...
{
	Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_LENSSTATE, 1);
}

void CdemoafDlg::SetLensStatus()
{
	Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_LENSSTATE, 0);
	Altaircam_get_LensInfo(m_hcam, &m_afLensInfo);
	m_slider_ap.SetPos(m_afLensInfo.posAM);
