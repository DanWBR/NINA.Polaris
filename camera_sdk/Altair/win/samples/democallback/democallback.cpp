...
public:
	...
	void OnDataEvent(bool bSnap)
	{
		if (bSnap)
		{
			...
			Altaircam_PullImageV4(m_hcam, NULL, 1, 24, 0, &info);
			...
			if (pSnapData)
			{
				Altaircam_PullImageV4(m_hcam, pSnapData, 1, 24, 0, NULL);
		...
		{
			{
				...
				Altaircam_PullImageV4(m_hcam, m_pData, 0, 24, 0, NULL);
			}
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Altaircam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Altaircam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Altaircam_StartPullModeWithCallback(m_hcam, StaticOnEventCallback, this);
					}
				}
			}
		}
