...
static bool IsSupportGain(HAltaircam h)
{
	unsigned short nMin = 0, nMax = 0;
	Altaircam_get_ExpoAGainRange(h, &nMin, &nMax, nullptr);
	return (nMax > nMin);
}
...
public:
	...
	: m_bSupportGain(bSupportGain), m_expoTimeRange(expoTimeRange), m_expoGainRange(expoGainRange), m_expoTime(0), m_expoGain(ALTAIRCAM_EXPOGAIN_MIN), m_delayTime(0)
	{
	}
...
class CConfigDlg : public CDialogImpl<CConfigDlg>
{
	...
	const AltaircamModelV2* m_pModel;
	const bool m_bSupportGain;
	HAltaircam	m_hcam;
...
public:
	enum { IDD = IDD_CONFIG };
	CConfigDlg(HAltaircam hcam, const AltaircamModelV2* pModel)
		: m_bSupportGain(IsSupportGain(hcam)), m_hcam(hcam), m_pModel(pModel), m_area(5), m_bin(1), m_temp(0), m_scale(0)
	{
		...
		Altaircam_get_ExpTimeRange(m_hcam, &m_expoTimeRange[0], &m_expoTimeRange[1], nullptr);
		Altaircam_get_ExpoAGainRange(m_hcam, &m_expoGainRange[0], &m_expoGainRange[1], nullptr);
	}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (Altaircam_get_MaxBitDepth(m_hcam) <= 8)
		...
		else
		{
			...
			Altaircam_get_PixelFormatSupport(m_hcam, -1, &num);
			for (int i = 0; i < num; ++i)
			{
				if (SUCCEEDED(Altaircam_get_PixelFormatSupport(m_hcam, (char)i, &pixelFormat)))
				{
					box.SetItemData(box.AddString(CA2W(Altaircam_get_PixelFormatName(pixelFormat))), pixelFormat);
		...
		if (E_NOTIMPL == Altaircam_get_Temperature(m_hcam, nullptr)) // support get the temperature of the sensor
...

		{
			...
			if (m_pModel->flag & ALTAIRCAM_FLAG_FAN)
			{
				box.AddString(L"OFF");
...

		{
			...
			if (m_pModel->flag & (ALTAIRCAM_FLAG_CG | ALTAIRCAM_FLAG_CGHDR))
			{
				DWORD val = 0;
				m_regkey.QueryDWORDValue(L"cg", val);
				if (m_pModel->flag & ALTAIRCAM_FLAG_GHOPTO)
				{
					box.AddString(L"LCG");
					box.AddString(L"MCG");
					box.AddString(L"HCG");
				...
				else
				{
					...
					box.AddString(L"LCG");
					box.AddString(L"HCG");
					if (m_pModel->flag & ALTAIRCAM_FLAG_CGHDR)
					{
						box.AddString(L"HDR");
						maxval = 2;
					}
		...
		if (m_pModel->flag & ALTAIRCAM_FLAG_TEC_ONOFF)
		{
			DWORD val = 0;
			...
			{
				int range = 0;
				Altaircam_get_Option(m_hcam, ALTAIRCAM_OPTION_TECTARGET_RANGE, &range);
		...
		if (m_pModel->flag & ALTAIRCAM_FLAG_HEAT)
		{
			CUpDownCtrl ctrl(GetDlgItem(IDC_SPIN8));
			DWORD heat = 0, maxheat = 0;
			Altaircam_get_Option(m_hcam, ALTAIRCAM_OPTION_HEAT_MAX, (int*)&maxheat);
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (m_pModel->flag & ALTAIRCAM_FLAG_FAN)
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_FAN, c);
		...
		if (m_pModel->flag & ALTAIRCAM_FLAG_TEC_ONOFF)
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TEC, bEnable);
			...
			if (bEnable)
			{
				...
				Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TECTARGET, n);
		...
		if (m_pModel->flag & ALTAIRCAM_FLAG_HEAT)
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_HEAT, heat);
		}
		...
		if (m_pModel->flag & (ALTAIRCAM_FLAG_CG | ALTAIRCAM_FLAG_CGHDR))
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_CG, c);
		...
		if (E_NOTIMPL != Altaircam_get_Temperature(m_hcam, nullptr))
		{
		...
		if (m_bin > 1)
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_BINNING, m_bin);
		}
		{
			const unsigned r = m_area / 2;
			int width = 0, height = 0;
			Altaircam_get_FinalSize(m_hcam, &width, &height);
			if (IsDlgButtonChecked(IDC_RADIO1))
			{
		...
		if (Altaircam_get_MaxBitDepth(m_hcam) > 8)
		{
			...
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_PIXEL_FORMAT, val);
...
public:
	...
	LRESULT OnStart(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (nullptr == m_hcam)
		{
			...
			else
			{
				...
				const unsigned num = Altaircam_EnumV2(arr);
				if (num <= 0)
	...
	void OnTimer(UINT_PTR nIDEvent)
	{
		...
		case TIMER_TEMP:
			...
			{
				short temp = 0;
				if (SUCCEEDED(Altaircam_get_Temperature(m_hcam, &temp)))
				{
					int val = temp;
...
private:
	void OpenCamera(const wchar_t* camId)
	{
		...
		m_pModel = Altaircam_query_Model(m_hcam);
		Altaircam_put_eSize(m_hcam, 0); // always use the maximum resolution
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_RAW, 1);
		Altaircam_put_HZ(m_hcam, 2);
		Altaircam_put_AutoExpoEnable(m_hcam, 0); // always disable auto exposure
		...
		else
		{
			...
			unsigned nFourCC;
			Altaircam_get_RawFormat(m_hcam, &nFourCC, &m_bitdepth);
			m_ymax = 1 << m_bitdepth;
			if (dlg.m_bin & 0x40)
			...
			int w = 0, h = 0;
			Altaircam_get_FinalSize(m_hcam, &w, &h);
			...
			m_curGraph = &m_vecGraph[0];
			m_curWnd = m_curGraph;
			if ((E_NOTIMPL != Altaircam_get_Temperature(m_hcam, nullptr)) && dlg.m_temp)
			{
				m_tabCtrl.AddItem(L"Temperature");
			...
			m_idxExpo = 0;
			Altaircam_put_ExpoTime(m_hcam, m_vecExpo[m_idxExpo].expoTime);
			if (m_bSupportGain)
				Altaircam_put_ExpoAGain(m_hcam, m_vecExpo[m_idxExpo].expoGain);
			Altaircam_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMERA);
			if (m_bTriggerMode)
			{
				Altaircam_Trigger(m_hcam, 1);
				m_bWantTigger = false;
	...
	template<typename T>
	void GetData(int* arr, const T* pData, const AltaircamFrameInfoV4& info)
	{
	...
	void OnEventImage()
	{
		...
		const HRESULT hr = Altaircam_PullImageV4(m_hcam, m_pRawData, 0, 0, 0, &info);
		if (SUCCEEDED(hr))
		{
			if (m_bTriggerMode)
				bAdd = true;
