...
public:
	...
	CExposureTimeDlg(HAltaircam hcam)
	: m_hcam(hcam)
	{
	}
private:
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (SUCCEEDED(Altaircam_get_ExpTimeRange(m_hcam, &nMin, &nMax, &nDef)))
		{
		...
		Altaircam_put_ExpoTime(m_hcam, nTime);
		EndDialog(wID);
		return 0;
	}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		ctrl.SetRangeMin(0);
		ctrl.SetRangeMax(Altaircam_get_MaxSpeed(m_hcam));

		unsigned short nSpeed = 0;
		if (SUCCEEDED(Altaircam_get_Speed(m_hcam, &nSpeed)))
			ctrl.SetPos(nSpeed);
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Altaircam_put_Speed(m_hcam, ctrl.GetPos());
...
private:
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (bTran1 && bTran2)
			Altaircam_put_MaxAutoExpoTimeAGain(m_hcam, nTime, nGain);
		EndDialog(wID);
		return 0;
	}
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Altaircam_put_LEDState(m_hcam, nIndex, 1, 0);
		return 0;
	}
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Altaircam_put_LEDState(m_hcam, nIndex, 2, nPeriod);
		return 0;
	}
	...
	{
		const UINT nIndex = GetDlgItemInt(IDC_EDIT1);
		Altaircam_put_LEDState(m_hcam, nIndex, 0, 0);
		return 0;
	}
...
static wchar_t* PixelFormatHdrName(int val)
{
	switch (val)
	{
	case ALTAIRCAM_PIXELFORMAT_HDR8HL:
		return L"HDR8HL";
	case ALTAIRCAM_PIXELFORMAT_HDR10HL:
		return L"HDR10HL";
	case ALTAIRCAM_PIXELFORMAT_HDR11HL:
		return L"HDR11HL";
	case ALTAIRCAM_PIXELFORMAT_HDR12HL:
		return L"HDR12HL";
	case ALTAIRCAM_PIXELFORMAT_HDR14HL:
		return L"HDR14HL";
	default:
		return nullptr;
	}
...
{
	const AltaircamDeviceV2&	m_tdev;
	HAltaircam				m_hcam;
...
public:
	...
	CPixelFormatDlg(const AltaircamDeviceV2& tdev, HAltaircam hcam)
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		CComboBox cbox(GetDlgItem(IDC_COMBO1));
		{
			...
			if (SUCCEEDED(Altaircam_get_PixelFormatSupport(m_hcam, -1, &num)) && (num > 0))
			{
				for (int i = 0; i < num; ++i)
				{
					if (SUCCEEDED(Altaircam_get_PixelFormatSupport(m_hcam, (char)i, &pixelFormat)))
						cbox.SetItemData(cbox.AddString(CA2W(Altaircam_get_PixelFormatName(pixelFormat))), pixelFormat);
				}
			}
		}

		{
			...
			Altaircam_get_Option(m_hcam, ALTAIRCAM_OPTION_PIXEL_FORMAT, &val);
			for (int i = 0; i < num; ++i)
			{
	...
	{
		CComboBox cbox(GetDlgItem(IDC_COMBO1));
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_PIXEL_FORMAT, cbox.GetItemData(cbox.GetCurSel()));
		return 0;
	}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		int val = 0;
		Altaircam_get_Option(m_hcam, ALTAIRCAM_OPTION_TECTARGET, &val);
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TECTARGET, (int)(d * 10));
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Altaircam_read_EEPROM(m_hcam, uAddr, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr = Altaircam_write_EEPROM(m_hcam, uAddr, tmpBuffer, uLength);
	...
	LRESULT OnButton3(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hrWrite = Altaircam_write_EEPROM(m_hcam, uAddr, tmpWriteBuffer, uLength);
				const HRESULT hrRead = Altaircam_read_EEPROM(m_hcam, uAddr, tmpReadBuffer, uLength);
	...
	LRESULT OnButton4(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr = Altaircam_write_EEPROM(m_hcam, uAddr, tmpWriteBuffer, uLength);
				if (hr == uLength)
				{
...
private:
	...
	LRESULT OnWmTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		if (1 == wParam)
		{
			if (S_OK == Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_STATUS, 0, 0, NULL))
				EndDialog(IDOK);
			else
...
public:
	enum { IDD = IDD_FLASH };
	CFlashDlg(HAltaircam hcam)
	: m_hcam(hcam)
	, m_totalSize(Altaircam_rwc_Flash(hcam, ALTAIRCAM_FLASH_SIZE, 0, 0, NULL))
	, m_eBlock(Altaircam_rwc_Flash(hcam, ALTAIRCAM_FLASH_EBLOCK, 0, 0, NULL))
	, m_rwBlock(Altaircam_rwc_Flash(hcam, ALTAIRCAM_FLASH_RWBLOCK, 0, 0, NULL))
	{
	}
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				{
					unsigned char* tmpBuffer = (unsigned char*)alloca(uLength);
					HRESULT hr = Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_READ, uAddr, uLength, tmpBuffer);
					if (FAILED(hr))
					{
						wchar_t strText[256];
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				else
				{
					...
					const HRESULT hr = Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_WRITE, uAddr, uLength, tmpBuffer);
					if (FAILED(hr))
					{
						wchar_t strText[256];
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				else
				{
					...
					for (unsigned i = 0; i < uLength; ++i)
						tmpWriteBuffer[i] = (unsigned char)rand();
					HRESULT hr = Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_WRITE, uAddr, uLength, tmpWriteBuffer);
					if (FAILED(hr))
					{
						swprintf(strText, L"Write failed, hr = 0x%08x", hr);
						AtlMessageBox(m_hWnd, strText, MB_OK | MB_ICONERROR);
					...
					else
					{
						...
						if (IDOK == dlg.DoModal())
						{
							hr = Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_READ, uAddr, uLength, tmpReadBuffer);
							if (FAILED(hr))
							{
								swprintf(strText, L"Read failed, hr = 0x%08x", hr);
								AtlMessageBox(m_hWnd, strText, MB_OK | MB_ICONERROR);
	...
	LRESULT OnButton4(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				{
					const HRESULT hr = Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_ERASE, uAddr, uLength, NULL);
					if (FAILED(hr))
					{
						wchar_t strText[256];
...
class CUARTDlg : public CDialogImpl<CUARTDlg>
{
	...
	BOOL		m_bUartEnable;
	BOOL		m_supportUart;
	BOOL		m_bUartLinemode;
	int			m_nUartBaudrate;
	BEGIN_MSG_MAP(CUARTDlg)
		MESSAGE_HANDLER(WM_INITDIALOG, OnInitDialog)
		COMMAND_HANDLER(IDC_CHECK1, BN_CLICKED, OnCheck1)
		COMMAND_HANDLER(IDC_COMBO1, CBN_SELENDOK, OnSelchange1)
		COMMAND_HANDLER(IDC_COMBO2, CBN_SELENDOK, OnSelchange2)
		COMMAND_HANDLER(IDC_BUTTON1, BN_CLICKED, OnButton1)
		COMMAND_HANDLER(IDC_BUTTON2, BN_CLICKED, OnButton2)
		COMMAND_HANDLER(IDCANCEL, BN_CLICKED, OnCancel)
	END_MSG_MAP()
public:
	enum { IDD = IDD_UART };
	CUARTDlg(HAltaircam hcam)
		: m_hcam(hcam), m_supportUart(FALSE), m_bUartEnable(FALSE), m_bUartLinemode(FALSE), m_nUartBaudrate(0)
	{
	}
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		CenterWindow(GetParent());
		m_supportUart = (E_NOTIMPL != Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_GET_UART_ENABLE, 0, nullptr));
		if (m_supportUart)
		{
			Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_ENABLE, 0, &m_bUartEnable);
			Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_BAUDRATE, 0, &m_nUartBaudrate);
			Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_LINEMODE, 0, &m_bUartLinemode);
 			CComboBox combo1(GetDlgItem(IDC_COMBO1));
			CComboBox combo2(GetDlgItem(IDC_COMBO2));
			combo1.AddString(L"9600");
			combo1.AddString(L"19200");
			combo1.AddString(L"38400");
			combo1.AddString(L"57600");
			combo1.AddString(L"115200");
			combo2.AddString(L"TX(GPIO_0)/RX(GPIO_1)");
			combo2.AddString(L"TX(GPIO_1)/RX(GPIO_0)");
			combo1.SetCurSel(m_nUartBaudrate);
			combo2.SetCurSel(m_bUartLinemode ? 1 : 0);

			CheckDlgButton(IDC_CHECK1, m_bUartEnable);
			GetDlgItem(IDC_EDIT1).EnableWindow(m_bUartEnable);
			GetDlgItem(IDC_BUTTON1).EnableWindow(m_bUartEnable);
			GetDlgItem(IDC_BUTTON2).EnableWindow(m_bUartEnable);
		}
		else
		{
	...
	LRESULT OnCheck1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (m_supportUart)
		{
			const BOOL bNewUartEnable = IsDlgButtonChecked(IDC_CHECK1);
			if (m_bUartEnable != bNewUartEnable)
			{
				m_bUartEnable = bNewUartEnable;
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_ENABLE, m_bUartEnable, nullptr);
			}
			GetDlgItem(IDC_EDIT1).EnableWindow(m_bUartEnable);
			GetDlgItem(IDC_BUTTON1).EnableWindow(m_bUartEnable);
			GetDlgItem(IDC_BUTTON2).EnableWindow(m_bUartEnable);
			::SendMessage(GetParent(), WM_COMMAND, MAKEWPARAM(IDC_COMBO1, CBN_SELCHANGE), (LPARAM)((CComboBox)GetDlgItem(IDC_COMBO1)).m_hWnd);
			::SendMessage(GetParent(), WM_COMMAND, MAKEWPARAM(IDC_COMBO2, CBN_SELCHANGE), (LPARAM)((CComboBox)GetDlgItem(IDC_COMBO2)).m_hWnd);
		}
		return S_OK;
	}
	...
	LRESULT OnSelchange1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		m_nUartBaudrate = ((CComboBox)GetDlgItem(IDC_COMBO1)).GetCurSel();
		if (m_nUartBaudrate < 0)
			m_nUartBaudrate = 0;
		else if (m_nUartBaudrate > 4)
			m_nUartBaudrate = 4;
		Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_BAUDRATE, m_nUartBaudrate, nullptr);
		return S_OK;
	}

	LRESULT OnSelchange2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		m_bUartLinemode = ((CComboBox)GetDlgItem(IDC_COMBO2)).GetCurSel();
		Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_UART_LINEMODE, m_bUartLinemode? 1 : 0, nullptr);
		return S_OK;
	}
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		const int maxEmptyRetry = 20;
		const int sleepMS = 10;
		int emptyRetry = 0;
		int bufSize = GetDlgItemInt(IDC_EDIT3);
		if (bufSize <= 0)
			return E_INVALIDARG;
		...
		do
		{
			HRESULT hr = Altaircam_read_UART(m_hcam, buffer, bufSize);
			ATLTRACE(L"hr = %d\n", hr);
			if (FAILED(hr))
			{
				AtlMessageBox(m_hWnd, L"Read UART failed!");
				break;
			}
			...
			emptyRetry = 0;
			allData.insert(allData.end(), buffer, buffer + hr);
			if (SUCCEEDED(hr))
				break;
		} while (1);
		...
		{
			std::wstringstream wstr;
			wstr << L"length = " << allData.size() << L", data = ";
			...
			SetDlgItemText(IDC_EDIT2, wstr.str().c_str());
		}
		else
		{
			AtlMessageBox(m_hWnd, L"No UART data (timeout).");
		}
		delete buffer;
		return S_OK;
	}
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		int len = GetDlgItemText(IDC_EDIT1, strData, _countof(strData));

		if (len > 0)
		{
			std::vector<unsigned char> tmpBuffer;
			tmpBuffer.reserve(len / 2);

			auto hex2int = [](wchar_t c) -> int
				{
			...
			while (i < len)
			{
				...
				int hi = hex2int(strData[i]);
				int lo = hex2int(strData[i + 1]);
				if (hi < 0 || lo < 0)
				{
					i++;
					continue;
				...
				tmpBuffer.push_back(val);
				i += 2;
			}

			if (!tmpBuffer.empty())
			{
				HRESULT hr = Altaircam_write_UART(m_hcam, tmpBuffer.data(), (unsigned)tmpBuffer.size());
				wchar_t strMessage[256];
				swprintf(strMessage, L"Write UART, length = %u, result = 0x%08x", (unsigned)tmpBuffer.size(), hr);
				AtlMessageBox(m_hWnd, strMessage);
			}
		}
		return 0;
...
public:
	...
	CIocontrolDlg(HAltaircam hcam, const AltaircamDeviceV2& tdev)
...
private:
	...
	{
		Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_RESETCOUNTER, 0, NULL);
		return 0;
	}
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		}
		{
			CComboBox box(GetDlgItem(IDC_GPIODIR));
			if (0 == box.GetCurSel())
				Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_GPIODIR, 0x00, NULL);
			else
				Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_GPIODIR, 0x01, NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTINVERTER));
			Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_OUTPUTINVERTER, box.GetCurSel(), NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_INPUTACTIVATION));
			Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_INPUTACTIVATION, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			if (!str.IsEmpty())
			{
				...
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_STROBEDURATION, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_DEBOUNCERTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_COUNTERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTMODE));
			Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_OUTPUTMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_USERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_STROBEDELAYMODE));
			Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_STROBEDELAYMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_STROBEDELAYTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_TRIGGERDELAY, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_PWMSOURCE));
			Altaircam_IoControl(m_hcam, index, ALTAIRCAM_IOCONTROLTYPE_SET_PWMSOURCE, box.GetCurSel(), NULL);
		}
		return 0;
	}
...
class CMainFrame : public CFrameWindowImpl<CMainFrame>, public CUpdateUI<CMainFrame>
{
	HAltaircam		m_hcam;
	CMainView		m_view;
	AltaircamDeviceV2	m_arrDev[ALTAIRCAM_MAX], m_dev;
	BOOL			m_bPaused;
...
private:
	...
	void EnumCamera()
	{
		...
		const unsigned cnt = Altaircam_EnumV2(m_arrDev);
		if (0 == cnt)
	...
	{
		Altaircam_GigeEnable(GigeHotplug, m_hWnd);
		EnumCamera();
	...
	{
		if (m_hcam)
			Altaircam_AwbOnce(m_hcam, NULL, NULL);
	}
	...
	void OnAutoExposure(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int mode = 0;
			if (SUCCEEDED(Altaircam_get_AutoExpoEnable(m_hcam, &mode)))
			{
				mode = !mode;
				Altaircam_put_AutoExpoEnable(m_hcam, mode);
	...
	void OnVerticalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Altaircam_get_VFlip(m_hcam, &b)))
			{
				b = !b;
				Altaircam_put_VFlip(m_hcam, b);
	...
	void OnHorizontalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Altaircam_get_HFlip(m_hcam, &b)))
			{
				b = !b;
				Altaircam_put_HFlip(m_hcam, b);
	...
	void OnPause(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			m_bPaused = !m_bPaused;
			Altaircam_Pause(m_hcam, m_bPaused);
	...
	{
		if (m_hcam && (m_dev.model->flag & ALTAIRCAM_FLAG_TEC_ONOFF)) // support set the tec target
		{
	...
	void OnFlash(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hcam)
		{
			if (Altaircam_rwc_Flash(m_hcam, ALTAIRCAM_FLASH_SIZE, 0, 0, NULL) <= 0)
	...
	void OnRoi(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			CRoiDlg dlg;
			Altaircam_get_Roi(m_hcam, &dlg.xOffset_, &dlg.yOffset_, &dlg.xWidth_, &dlg.yHeight_);
			if (IDOK == dlg.DoModal())
			{
				if (SUCCEEDED(Altaircam_put_Roi(m_hcam, dlg.xOffset_, dlg.yOffset_, dlg.xWidth_, dlg.yHeight_)))
				{
					Altaircam_get_Roi(m_hcam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Altaircam_get_FwVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Altaircam_get_HwVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Altaircam_get_FpgaVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	void OnTestPattern(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (val)
			val = val * 2 + 1;
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TESTPATTERN, val);
	}

	void OnProductionDate(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		char pdate[10] = { 0 };
		if (SUCCEEDED(Altaircam_get_ProductionDate(m_hcam, pdate)))
		{
			CA2T a2t(pdate);
	...
	{
		char sn[32] = { 0 };
		if (SUCCEEDED(Altaircam_get_SerialNumber(m_hcam, sn)))
		{
			CA2T a2t(sn);
	...
	void OnRawformat(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (SUCCEEDED(Altaircam_get_RawFormat(m_hcam, &nFourCC, &bitsperpixel)))
		{
			wchar_t str[257];
	...
	void OnTriggerMode(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int val = 0;
			Altaircam_get_Option(m_hcam, ALTAIRCAM_OPTION_TRIGGER, &val);
			if (val == 0)
				val = (m_dev.model->flag & ALTAIRCAM_FLAG_TRIGGER_EXTERNAL) ? 2 : 1;
			else
				val = 0;
			Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TRIGGER, val);
	...
	void OnTriggerNumber(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (m_dev.model->ioctrol > 0)
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_BURSTCOUNTER, m_nTriggerNumber, NULL);
		}
	}
	...
	void OnTriggerTrigger(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hcam)
		{
			...
			{
				m_eTriggerType = eTriggerNumber;
				Altaircam_IoControl(m_hcam, 0, ALTAIRCAM_IOCONTROLTYPE_SET_TRIGGERSOURCE, 5, NULL);
				const HRESULT hr = Altaircam_Trigger(m_hcam, m_nTriggerNumber);
				if (E_INVALIDARG == hr)
				{
					if (m_nTriggerNumber > 1)
						AtlMessageBox(m_hWnd, L"ALTAIRCAM_FLAG_TRIGGER_SINGLE: only number = 1 supported");
				}
			}
		}
	}
	...
	void OnPreviewResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		unsigned eSize = 0;
		if (SUCCEEDED(Altaircam_get_eSize(m_hcam, &eSize)))
		{
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Altaircam_Stop(m_hcam)))
				{
					...
					Altaircam_put_eSize(m_hcam, nID - ID_PREVIEW_RESOLUTION0);
					for (unsigned i = 0; i < m_dev.model->preview; ++i)
						UISetCheck(ID_PREVIEW_RESOLUTION0 + i, (nID - ID_PREVIEW_RESOLUTION0 == i) ? 1 : 0);
					UpdateSnapMenu();
					if (SUCCEEDED(Altaircam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						UpdateResolutionText();
						...
						if (SUCCEEDED(Altaircam_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMEVENT)))
						{
	...
	void OnSnapResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (SUCCEEDED(Altaircam_Snap(m_hcam, nID - ID_SNAP_RESOLUTION0)))
			{
				m_nSnapType = 1;
				m_nSnapSeq = 0;
				UpdateSnapMenu();
	...
	void OnSnapnResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		{
			if (SUCCEEDED(Altaircam_SnapN(m_hcam, nID - ID_SNAPN_RESOLUTION0, dlg.m_nNum)))
			{
				m_nSnapType = 2;
	...
	void OnOpenDevice(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		m_hcam = Altaircam_Open(m_arrDev[idx].id);
		if (m_hcam)
		{
			m_dev = m_arrDev[idx];
			...
			{
				Altaircam_put_Roi(m_hcam, m_xRoiOffset, m_yRoiOffset, m_xRoiWidth, m_yRoiHeight);
				Altaircam_get_Roi(m_hcam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
			}
			else
			{
				Altaircam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight);
			}

			if (m_bTriggerMode)
			{
				if (m_dev.model->flag & ALTAIRCAM_FLAG_TRIGGER_EXTERNAL)
					Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TRIGGER, 2);
				else if (m_dev.model->flag & ALTAIRCAM_FLAG_TRIGGER_EXTERNAL)
					Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_TRIGGER, 1);
			}
	...
	void OnEventImage()
	{
		...
		HRESULT hr = Altaircam_PullImageV4(m_hcam, m_pData, 0, m_header.biBitCount, 0, &info);
		if (FAILED(hr))
			return;
	...
	void OnEventSnap()
	{
		...
		AltaircamFrameInfoV4 info = { 0 };
		HRESULT hr = Altaircam_PullImageV4(m_hcam, NULL, 1, 24, 0, &info); //first, peek the width and height
		if (SUCCEEDED(hr))
		{
			...
			if (pSnapData)
			{
				hr = Altaircam_PullImageV4(m_hcam, pSnapData, 1, 24, 0, NULL);
				if (SUCCEEDED(hr))
				{
					if (2 == m_nSnapType)
					{
	...
	{
		wchar_t res[128];
		int nTemp = ALTAIRCAM_TEMP_DEF, nTint = ALTAIRCAM_TINT_DEF;
		Altaircam_get_TempTint(m_hcam, &nTemp, &nTint);
	...
	void OnEventExpo()
	{
		...
		if (SUCCEEDED(Altaircam_get_ExpoTime(m_hcam, &nTime)) && SUCCEEDED(Altaircam_get_ExpoAGain(m_hcam, &Gain)))
		{
	...
	void OnDeviceChanged()
	{
		...
		{
			unsigned eSize = 0;
			Altaircam_get_eSize(m_hcam, &eSize);
			...
			int nTemp = ALTAIRCAM_TEMP_DEF, nTint = ALTAIRCAM_TINT_DEF;
			if (SUCCEEDED(Altaircam_get_TempTint(m_hcam, &nTemp, &nTint)))
			{
				swprintf(res, L"Temp = %d, Tint = %d", nTemp, nTint);
		...
		UIEnable(ID_TECTARGET, (m_hcam && (m_dev.model->flag & ALTAIRCAM_FLAG_TEC_ONOFF)) ? TRUE : FALSE);
	...
	void UpdateResolutionText()
	{
		...
		if (SUCCEEDED(Altaircam_get_Roi(m_hcam, &xOffset, &yOffset, &nWidth, &nHeight)))
		{
	...
	void UpdateFrameText()
	{
		...
		if (m_hcam && SUCCEEDED(Altaircam_get_FrameRate(m_hcam, &nFrame, &nTime, &nTotalFrame)))
		{
			wchar_t str[256];
			if (nTime >= 1000)
	...
	void UpdateFrameInfoText(const AltaircamFrameInfoV4& info)
	{
		wchar_t str[256];
