...
CD3D11Render::CD3D11Render(HAltaircam hcam, HWND hwndTarget, int imageWidth, int imageHeight)
: m_hcam(hcam), m_bMono(Altaircam_get_MonoMode(m_hcam) == S_OK), m_hwnd(hwndTarget), m_imageWidth(imageWidth), m_imageHeight(imageHeight)
...
{
	Altaircam_get_RawFormat(m_hcam, NULL, &m_bitdepth);
}
...
void CD3D11Render::Resize()
{
	...
	if ((m_windowWidth != rc.right - rc.left) || (m_windowHeight != rc.bottom - rc.top))
	{
		...
		ALTAIRCAM_LOG_VERBOSE("%s: %d %d", __func__, cx, cy);
...
{
	ALTAIRCAM_LOG_VERBOSE("%s: %d %d", __func__, windowWidth, windowHeight);
...
{
	ALTAIRCAM_LOG_VERBOSE("%s", __func__);
	SetEvent(m_evt);
}
...
void CD3D11Render::Loop()
{
	while (m_loop)
	{
		...
		if (SUCCEEDED(hr))
		{
			hr = Altaircam_PullImageWithRowPitch(m_hcam, mapped.pData, m_bMono ? ((m_bitdepth > 8) ? 16 : 8) : ((m_bitdepth > 8) ? 64 : 32), mapped.RowPitch, NULL, NULL);
			if (SUCCEEDED(hr))
		...
		if (SetupRtv())
		{
			...
			ALTAIRCAM_LOG_VERBOSE("%s: Present, 0x%08x, tick = %u", __func__, hr, get_precise_tick() - tick);
		}
	}
}
