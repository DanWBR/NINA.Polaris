...
BEGIN_MESSAGE_MAP(Cdemod3d11Dlg, CDialog)
	...
	ON_WM_GETMINMAXINFO()
	ON_WM_TIMER()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + ALTAIRCAM_MAX, &Cdemod3d11Dlg::OnSnapResolution)
END_MESSAGE_MAP()
...
void Cdemod3d11Dlg::OnBnClickedButton1()
{
	...
	Altaircam_put_RealTime(m_hcam, 1);
	Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_BYTEORDER, 0); //RGB or BGR
...
void Cdemod3d11Dlg::StartDevice()
{
	...
	if (IsDlgButtonChecked(IDC_CHECK2))
	{
		...
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_RGB, Altaircam_get_MonoMode(m_hcam) ? 5 : 4);	// RGB64
	}
	else
	{
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_BITDEPTH, 0);
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_RGB, Altaircam_get_MonoMode(m_hcam) ? 2 : 3);	// RGB32
	}
...
void Cdemod3d11Dlg::OnEventStillImage()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = Altaircam_PullImageV3(m_hcam, pData, 1, 24, 0, NULL);
		if (SUCCEEDED(hr))
		{
...
void Cdemod3d11Dlg::OnTimer(UINT_PTR nIDEvent)
{
	if ((1 == nIDEvent) && m_hcam && m_render)
	{
		...
		if (SUCCEEDED(Altaircam_get_FrameRate(m_hcam, &nFrame, &nTime, &nTotalFrame)) && nTime)
		{
