...
BEGIN_MESSAGE_MAP(CdemomonoDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + ALTAIRCAM_MAX, &CdemomonoDlg::OnSnapResolution)
END_MESSAGE_MAP()
...
void CdemomonoDlg::OnBnClickedButton1()
{
	...
	int n = (int)Altaircam_get_ResolutionNumber(m_hcam);
	if (n > 0)
	{
		TCHAR txt[128];
		int nWidth, nHeight;
	...
	m_maxBitDepth = Altaircam_get_MaxBitDepth(m_hcam);

	if ((8 == m_maxBitDepth) || IsDlgButtonChecked(IDC_CHECK2))
	{
		m_bBitDepth = false;
		CheckDlgButton(IDC_CHECK2, 0);
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_RGB, 3);
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_BITDEPTH, 0);
	}
	else
	{
		m_bBitDepth = true;
		Altaircam_put_Option(m_hcam, ALTAIRCAM_OPTION_RGB, 4);
...
void CdemomonoDlg::OnEventImage()
{
	...
	HRESULT hr = Altaircam_PullImageV4(m_hcam, m_pImageData, 0, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
...
{
	AltaircamFrameInfoV4 info = { 0 };
	HRESULT hr = Altaircam_PullImageV4(m_hcam, NULL, 1, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.v3.width * 16) * info.v3.height);
		hr = Altaircam_PullImageV4(m_hcam, pData, 1, m_bBitDepth ? 16 : 8, 0, NULL);
		if (SUCCEEDED(hr))
		{
