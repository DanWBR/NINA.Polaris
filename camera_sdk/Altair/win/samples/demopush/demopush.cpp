...
public:
	...
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (SUCCEEDED(Altaircam_get_ExpTimeRange(m_hcam, &nMin, &nMax, &nDef)))
		{
			...
			if (SUCCEEDED(Altaircam_get_ExpoTime(m_hcam, &nTime)))
				ctrl.SetPos(nTime);
		}
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Altaircam_put_ExpoTime(m_hcam, ctrl.GetPos());
...
class CMainFrame : public CFrameWindowImpl<CMainFrame>, public CUpdateUI<CMainFrame>
{
	HAltaircam			m_hcam;
	CMainView			m_view;
	AltaircamDeviceV2		m_dev[ALTAIRCAM_MAX];
	unsigned			m_nIndex;
	bool				m_bSnap;
...
public:
	...
	int OnCreate(LPCREATESTRUCT /*lpCreateStruct*/)
	{
		...
		unsigned cnt = Altaircam_EnumV2(m_dev);
		if (0 == cnt)
	...
	void OnDataCallbackV4(const void* pData, const AltaircamFrameInfoV3* pInfo, int bSnap)
	{
		if (NULL == pData)
			PostMessage(MSG_CAMEVENT, ALTAIRCAM_EVENT_ERROR);
		else if (bSnap)
		{
	...
	static void __stdcall StaticOnDataCallbackV4(const void* pData, const AltaircamFrameInfoV3* pInfo, int bSnap, void* pCallbackCtx)
	{
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Altaircam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Altaircam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Altaircam_StartPushModeV4(m_hcam, StaticOnDataCallbackV4, this, StaticOnEventCallback, this);
					}
				}
			}
		}
	...
	void OnOpenDevice(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		m_hcam = Altaircam_Open(m_dev[m_nIndex].id);
		if (m_hcam)
		{
