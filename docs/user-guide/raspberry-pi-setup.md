...
|---|---|---|---|
| RAM | 2 GB minimum, 4 GB recommended | 2 GB minimum, 4 GB recommended | Runs on as little as 1.5 GB (Orange Pi Zero 3); below 2 GB, live stacking leans on Auto to pick a smaller bin and the STUDIO batch stacker gets tight. 8 GB for STUDIO jobs during a live session, or to run the AI Assistant local backend (its model needs ~2.5 GB on top of Polaris). |
