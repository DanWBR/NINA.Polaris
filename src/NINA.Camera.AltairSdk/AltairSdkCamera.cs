...
/// <summary>
/// <see cref="ICamera"/> backend for Altair cameras over the native altaircam
/// SDK (via the vendored official <c>Altaircam</c> binding). Same shape as the
/// SVBony / ZWO / PlayerOne backends: full still capture + cooler + ROI, plus
/// native video streaming. Altair's SDK is callback-driven (pull mode): we
...
/// the native path. This is the high-fps path for Altair planetary cams.
/// </summary>
public sealed class AltairSdkCamera : ICamera {
    ...
    private Altaircam.DelegateEventCallback? _evtCb;
    ...
    public AltairSdkCamera(string deviceId) {
        AltairRegistry.EnsureResolver();
        _camId = deviceId ?? string.Empty;
        DeviceName = $"Altair {_camId}";
    }
    ...
    public bool CoolerOn {
        get {
            if (_cam != null && _supportsCooler && _cam.get_Option(Altaircam.eOPTION.OPTION_TEC, out int v)) return v != 0;
            return false;
        }
    }
    ...
    public Task ConnectAsync(CancellationToken ct = default) => Task.Run(() => {
        ...
        ulong flag = 0;
        try {
            foreach (var d in Altaircam.EnumV2()) {
                if (d.id == _camId) {
                    flag = d.model.flag;
        ...
        var cam = Altaircam.Open(_camId);
        if (cam == null) throw new InvalidOperationException(
            $"Failed to open the Altair camera '{_camId}'. The camera may already " +
        ...
        _isColor = (flag & (ulong)Altaircam.eFLAG.FLAG_MONO) == 0;
        _supportsCooler = (flag & (ulong)(Altaircam.eFLAG.FLAG_TEC | Altaircam.eFLAG.FLAG_TEC_ONOFF)) != 0;

        // Raw (bayer) output, max bit depth.
        cam.put_Option(Altaircam.eOPTION.OPTION_RAW, 1);
        cam.put_Option(Altaircam.eOPTION.OPTION_BITDEPTH, 1);
        ...
        // Exposure range, in MICROSECONDS per the altaircam SDK.
    ...
    public Task SetTemperatureAsync(double temperature, CancellationToken ct = default) {
        if (_cam != null && _supportsCooler)
            _cam.put_Option(Altaircam.eOPTION.OPTION_TECTARGET, (int)Math.Round(temperature * 10));
        return Task.CompletedTask;
    }

    public Task SetCoolerAsync(bool on, CancellationToken ct = default) {
        if (_cam != null && _supportsCooler)
            _cam.put_Option(Altaircam.eOPTION.OPTION_TEC, on ? 1 : 0);
        return Task.CompletedTask;
    }
    ...
    public IReadOnlyList<CameraControl> GetControls() {
        ...
        if (_cam.get_Option(Altaircam.eOPTION.OPTION_BLACKLEVEL, out int bl))
        ...
        if (_cam.get_Gamma(out int gm))
            list.Add(new CameraControl("Gamma", "Gamma", "Tone curve (100 = linear)",
                gm, Altaircam.GAMMA_MIN, Altaircam.GAMMA_MAX, Altaircam.GAMMA_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
        ...
        if (_isColor && _cam.get_Saturation(out int sat))
            list.Add(new CameraControl("Saturation", "Saturation", "Colour saturation (128 = neutral)",
                sat, Altaircam.SATURATION_MIN, Altaircam.SATURATION_MAX, Altaircam.SATURATION_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
        ...
        if (_cam.get_Contrast(out int ct))
            list.Add(new CameraControl("Contrast", "Contrast", "Display contrast (0 = neutral)",
                ct, Altaircam.CONTRAST_MIN, Altaircam.CONTRAST_MAX, Altaircam.CONTRAST_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
        if (_cam.get_Brightness(out int br))
            list.Add(new CameraControl("Brightness", "Brightness", "Display brightness (0 = neutral)",
                br, Altaircam.BRIGHTNESS_MIN, Altaircam.BRIGHTNESS_MAX, Altaircam.BRIGHTNESS_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
        ...
        if (_isColor && _cam.get_TempTint(out int wbTemp, out int wbTint)) {
            list.Add(new CameraControl("WBTemp", "WB temperature", "White-balance colour temperature (K)",
                wbTemp, Altaircam.TEMP_MIN, Altaircam.TEMP_MAX, Altaircam.TEMP_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
            list.Add(new CameraControl("WBTint", "WB tint", "White-balance tint",
                wbTint, Altaircam.TINT_MIN, Altaircam.TINT_MAX, Altaircam.TINT_DEF,
                Writable: true, Auto: false, AutoSupported: false, "int"));
        }
        // In-camera denoise strength. Keep at 0 for stacking (denoise later).
        if (_cam.get_Option(Altaircam.eOPTION.OPTION_DENOISE, out int dn))
        ...
        if (fanMax > 0 && _cam.get_Option(Altaircam.eOPTION.OPTION_FAN, out int fan))
        ...
        try {
            switch (id) {
                ...
                case "Offset":
                    ...
                    return _cam.put_Option(Altaircam.eOPTION.OPTION_BLACKLEVEL, _offset);
                case "Gamma":
                ...
                case "Denoise":
                    return _cam.put_Option(Altaircam.eOPTION.OPTION_DENOISE, (int)Math.Round(value));
                case "Fan":
                    return _cam.put_Option(Altaircam.eOPTION.OPTION_FAN, (int)Math.Round(value));
                case "Cooler":
    ...
    public Task<IImageData> CaptureAsync(double exposureSeconds, CaptureOptions? opts = null,
        ...
        try {
            ...
            if (!tcs.Task.Wait(waitMs, ct))
                throw new TimeoutException("Altair frame timed out.");
    ...
    private void OnEvent(Altaircam.eEVENT e) {
        ...
        try {
            if (_bitDepth > 8) {
                ...
                if (!_cam.PullImage(_buf16, 0, 16, 0, out Altaircam.FrameInfoV4 _)) return;
                frame = WrapFrame16(_buf16, w, h);
            } else {
                if (_buf8 == null || _buf8.Length < (long)w * h) _buf8 = new byte[(long)w * h];
                if (!_cam.PullImage(_buf8, 0, 8, 0, out Altaircam.FrameInfoV4 _)) return;
    ...
    private void ApplyExposureGain(double exposureSeconds, int? gainOverride, int? offsetOverride = null) {
        ...
        if (offsetOverride is int o) {
            ...
            _cam.put_Option(Altaircam.eOPTION.OPTION_BLACKLEVEL, _offset);
        }
    }
    ...
    /// <summary>Map a Altair raw FourCC (e.g. 'GBRG') to a Bayer pattern.
    ...
    private sealed class Sub : IDisposable {
        private readonly AltairSdkCamera _cam; private readonly int _id;
        public Sub(AltairSdkCamera cam, int id) { _cam = cam; _id = id; }
