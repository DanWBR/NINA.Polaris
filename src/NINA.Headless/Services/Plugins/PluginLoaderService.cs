...
/// add a reload endpoint), and invokes <see cref="INinaHeadlessPlugin.Register"/>
    ...
    private void TryLoadPlugin(string path) {
        try {
            ...
            var pluginTypes = asm.GetTypes()
                .Where(t => typeof(INinaHeadlessPlugin).IsAssignableFrom(t)
                         && !t.IsAbstract && !t.IsInterface)
                .ToArray();
            if (pluginTypes.Length == 0) {
                _logger.LogDebug("{Path}: no INinaHeadlessPlugin implementations", path);
                return;
            }
            foreach (var t in pluginTypes) {
                try {
                    var plugin = (INinaHeadlessPlugin?)Activator.CreateInstance(t)
