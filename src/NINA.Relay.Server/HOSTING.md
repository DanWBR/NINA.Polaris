...
| **B. Project-run shared relay** | You run *one* multi-tenant instance, issue tokens | ~$5 to $15/mo (see cost model) | None (zero-config) | The ZWO model. Code is ready; cost is dominated by bandwidth |
| **C. Managed third-party tunnel** | Cloudflare Tunnel / Tailscale Funnel / ngrok / frp | $0 to low | Medium | Offloads bandwidth, TLS, DDoS; ToS limits on heavy streaming; per-user accounts add friction |
...
- Casual monitoring (status stream + a preview JPEG per sub): **~50 to 200 MB/night**.
- Continuous live view / focus loop (JPEG ~200 KB at ~1 fps): **~0.7 GB/hour**:
  this is the blow-up case.
- Pulling a full-resolution light: **30 to 60 MB each**.
...
| OVH / DigitalOcean | ~$6/mo | 1 to 2 TB | **covered** (~$6) |
| Cloudflare Tunnel in front | $0 egress | effectively unlimited* | **~$0** (*subject to ToS) |
| AWS / GCP / Azure | egress $0.08 to $0.12/GB | ~0 | **~$90/mo** |
...
shared relay for **hundreds of users costs on the order of $5 to $15/month**. On a
...
(e.g. 20 to 50 GB/mo) in `tenants.json`. Over-quota tenants get HTTP 402 and their
...
1. **Phase 1: one shared opt-in instance (Option B), cheap.** Run a single
   ...
   opt-in convenience tier. This already delivers the ZWO-style UX at ~$5 to $15/mo.
