...
public class SequenceEngineDitherTests {
    ...
    private SequenceEngine MakeEngine() {
        ...
        // Isolate the profile in a throwaway temp dir: the engine now persists +
        // restores its schedule/dither per rig, so sharing %LOCALAPPDATA% would
        // leak state between tests (and the real machine's profile).
